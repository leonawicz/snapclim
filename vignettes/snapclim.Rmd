...
If at any time you get stuck with using `climdata`, see the function documentation.
It provides detailed descriptions and usage for the available function arguments.

SNAP climate data sets are accessed with `climdata`. The first argument, `id`, specifies a unique data collection.
See the `id` column in `collections` above. Next, a location is specified. 
...
Since the columns are truncated to fit in the display below, the first seven columns of ID variables are dropped here.
...
If you simply change this to `seasonal` or `annual`, `climdata` will return the respective data set.

```{r data5}
climdata(id = "ar5stats", area = "Arctic Tundra", time_scale = "seasonal")
```
...
```{r data6}
climdata(id = "ar5stats", area = "Arctic Tundra", time_scale = "seasonal", decavg = TRUE)
```
...
By specifying `area = "points"` rather than a specific point location, a table is returned containing data all 3,867 locations. 
